...
DIR_ORIGEM="/etc"
DIR_DESTINO="/mnt/nas-sede-incremental"
NOME_COMPARTILHAMENTO="nas-sede-incremental"
...
# MARCA O HORÁRIO DE INÍCIO E NOMEIA ARQUIVO DE FORMA ÚNICA
DATA_INICIO=$(date +"%d/%m/%Y às %H:%M:%S")
DATA=$(date +%Y-%m-%d)
LOG="/tmp/BKP-INC-${DATA}.txt"

API_URL="http://clubenaval337.clubenaval.intra:5000/api/backup"
DESTINATARIO="servidores@cnsocial.org.br"

ASSUNTO1="Backup INCREMENTAL do Servidor $(hostname | cut -d. -f1) para o $(ssh $STORAGE "hostname -s" 2>/dev/null) em $(date +%d/%m/%Y)"
ASSUNTO2="Erro no Backup do servidor $(hostname | cut -d. -f1) em $(date +%d/%m/%Y)"
TEMPS="$LOG /root/sent"
...
if [ "$verifica_montagem_destino" = "$NOME_COMPARTILHAMENTO" ]
  then
    ...
    {
        echo "<html><body><h3>$ASSUNTO1<br>Confira os LOGS em anexo</h3><table border='1' cellpadding='5' cellspacing='0' style='border-collapse: collapse;'>"
    ...
    mutt -e "set content_type=text/html" -s "$ASSUNTO1" -a "$LOG" -- "$DESTINATARIO" < "$ARQUIVO_TEMP"
    rm -f "$ARQUIVO_TEMP"
        ...
        erro_msg="Compartilhamento NAO MONTADO! O Backup INCREMENTAL do dia $DATA NAO FOI FEITO!"
        echo "$erro_msg" > "$LOG"
